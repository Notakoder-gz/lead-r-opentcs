...
def main():
    ...
    # Создаем объект стартовой позиции (текущее время RMF, индекс waypoint=0, угол=0.0)
    # Настоящие координаты прилетят из Zenoh чуть позже
    starts = [plan.Start(adapter.now(), 0, 0.0)]

    # Функция-коллбэк для первого робота (сработает, когда RMF одобрит регистрацию)
    ...
    # Функция-коллбэк для второго робота
